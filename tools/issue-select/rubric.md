...
| Newcomer-sized scope           | Issue body and comment thread, including any acceptance criteria, maintainer clarification, linked attempts, and design discussion.                    | Pass if the issue asks for one bounded contribution that can be implemented and tested without unresolved design decisions or stated changes to core internals. Fail if it is an umbrella or tracking issue, a pure usage question, or the requested design is still being debated without a maintainer decision. | required  |
| Issue available                | Repo-facts block: this issue's assignees and linked PR states; issue comment thread for claim comments and PR mentions.                                | Pass if the issue has no assignee, no open linked or mentioned pull request, and no active claim confirmed by a maintainer. A closed unmerged PR or an unanswered request to work on the issue does not fail this check.                                                                                          | required  |
